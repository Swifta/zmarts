...
<div class="cont_container mt15 mt10">
	...
	<form method="get" class="admin_form" action=""> 
		...
		<?php if(count($this->store_credits_transaction_list)>0){
                ...
                <?php if($this->pagination !=""){ ?>
                        <a class="fr frm_export" href="<?php echo PATH.$this->sort_url.'?id='.$this->Lang['SEARCH'].'&name='.$this->input->get('name').$parm.'&today='.$this->input->get('today').'&startdate='.$this->input->get('startdate').'&enddate='.$this->input->get('enddate'); ?>" title="<?php echo $this->Lang['EXP_DL']; ?>"><?php echo $this->Lang['EXP_PAGE']; ?></a>
                <?php } ?>			
	...
	<table class="list_table1 fl clr">
        ...
        <div class="date_range_common">            
            ...
            <div class="list_table1 fl clr date_range_part">
		        ...
		        <option value="<?php echo PATH.'merchant-storecredits/all-transaction.html';?>" <?php if($this->uri->last_segment() == "all-transaction.html"){  ?>  selected="selected" <?php }  ?>><?php echo $this->Lang["ALL_TRAN"]; ?></option>

		         <option value="<?php echo PATH.'merchant-storecredits/success-transaction.html';?>" <?php if(($this->uri->last_segment() == "success-transaction.html")||($this->uri->segment(2)== "success-transaction")){  ?>  selected="selected" <?php }  ?>><?php echo $this->Lang["COMP_TRAN"]; ?></option>
		        
		        <option value="<?php echo PATH.'merchant-storecredits/failed-transaction.html';?>" <?php if(($this->uri->last_segment() == "failed-transaction.html")||($this->uri->segment(2)== "failed-transaction")){  ?>  selected="selected" <?php }  ?>><?php echo $this->Lang["FAI_TRAN"]; ?></option>
		        
		        <option value="<?php echo PATH.'merchant-storecredits/hold-transaction.html';?>" <?php if(($this->uri->last_segment() == "hold-transaction.html")||($this->uri->segment(2)== "hold-transaction")){  ?>  selected="selected" <?php }  ?>><?php echo $this->Lang["HOLD_TRAN"]; ?></option>         
		        </select></span>
		        */ ?>	
                </div>
            </div>
	...
	<?php echo new View("admin_product/store_credits_transaction_list"); ?>
    </div>
