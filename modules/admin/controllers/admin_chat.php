...
class Admin_chat_Controller extends website_Controller {
	...
	public function __construct()
	{
		...
		$this->chat_act =1;
		$this->chat = new Admin_chat_Model();	
	...
	public function offline_chat_history($page="")
	{
		...
		$this->url="admin/offline-chat.html"; // for export
		...
		if($_POST){
			...
			if($post->validate()){
				...
				if($lastsession){
					...
					url::redirect(PATH."admin/offline-chat.html");
				}
		...
		$this->count_user = $this->chat->get_offline_count($this->input->get("search"));
				...
				'base_url'       => 'admin/offline-chat.html/page/'.$page."/",
				'uri_segment'    => 4,
		...
		$this->template->content = new View("admin_chat/offline_chat");
	}
	...
	public function online_chat_history($page="")
	{
		...
		if($_POST){
			...
			if($post->validate()){
				...
				common::message(1,$this->Lang["MAIL_SENDED"]);
				url::redirect(PATH."admin/online-chat.html");
			}
			else{
				$this->form_error = error::_error($post->errors());
			}
		...
		$this->template->content = new View("admin_chat/online_chat");
	...
	/* View chat History */
	 public function view_chat_history($sellerid ="",$from_name="")
	 {
		...
		$this->template->content = new View("admin_chat/view_chat_history");
	 ...
	 public function view_offlinereply($offlineid ="")
	 {
		...
		$this->template->content = new View("admin_chat/view_offline_history");
