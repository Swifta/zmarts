...
class Admin_customer_care_Controller extends website_Controller {
	...
	public function __construct()
	{
		...
		$this->users = new Admin_customer_care_Model();	
		$this->customer_care_act = "1";	
	}
	...
	{
		if(!ADMIN_PRIVILEGES_CUSTOMERCARE_ADD)
		{
		...
		$this->template->title = $this->Lang["ADD_CUSTOMER_CARE"];
		$this->template->content = new View("admin_customer_care/add_customer_care");
	}
	...
	{
		if(!ADMIN_PRIVILEGES_CUSTOMERCARE)
		{
			common::message(-1, $this->Lang["YOU_CAN_NOT_MODULE"]);        
			url::redirect(PATH."admin.html");
		}
		...
		$this->url="admin/manage-customer-care.html"; // for export
        ...
        $this->pagination = new Pagination(array(
            'base_url'       => 'admin/manage-customer-care.html/page/'.$page."/",
            'uri_segment'    => 4,
        ...
        $this->users_list = $this->users->get_users_list($this->pagination->sql_offset, $this->pagination->items_per_page, $this->input->get('name'), $this->input->get('email'), $this->input->get('city'), $this->input->get('logintype'),$this->type,$this->sort,0,$this->today,$this->startdate,$this->enddate);
			...
			if($search){
				...
				foreach($this->users_list as $d)
				{
					...
					header('Content-Disposition: attachment; filename=Customercare.xls');
                ...
                $this->template->title = $this->Lang["MANAGE_CUSTOMER_CARE"];
                $this->template->content = new View("admin_customer_care/manage_customer_care");
	}
	...
	{
		if(!ADMIN_PRIVILEGES_CUSTOMERCARE_EDIT)
		{
			common::message(-1, $this->Lang["YOU_CAN_NOT_MODULE"]);        
			url::redirect(PATH."admin.html");
		}
		...
		$this->template->content = new View("admin_customer_care/edit_customer_care");
	}
	...
	{
		if(!ADMIN_PRIVILEGES_CUSTOMERCARE_BLOCK)
		{
			common::message(-1, $this->Lang["YOU_CAN_NOT_MODULE"]);        
			url::redirect(PATH."admin.html");
		}
	...
	public function view_user($user_id = "")
	{
		...
		$this->template->title = $this->Lang["USER_CARE_DET"];
		$this->template->content = new View("admin_customer_care/view_customer_care");
	}
	...
	public function dashboard_users()
	{
		...
		$this->dashboard_customer_care = "1";
