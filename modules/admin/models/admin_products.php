...
//			$query = "select * , product.created_date as createddate from product 
...
//			$query = "select product.deal_id from product
//                            join stores on stores.store_id=product.shop_id 
//                            join city on city.city_id=stores.city_id  
//                            join category on category.category_id=product.category_id
...
//                join stores on stores.store_id=product.shop_id 
//                WHERE purchase_count < user_limit_quantity and deal_status = 1 and stores.store_status = 1");
        ...
        public function get_users_comments_list($offset = "", $record = "",  $firstname = "",$deal_type = "")
        {
                $contitions = "discussion.type = $deal_type ";
                        ...
                        $contitions .= 'OR discussion.comments like "%'.strip_tags(addslashes($firstname)).'%")';
                          ...
                          ->join("product","product.deal_id","discussion.deal_id")
...
//                            join product on product.deal_id=discussion.deal_id where $contitions order by discussion_id DESC limit $offset, $record");
                        return $result;
        }
...
//                       $result = $this->db->query("select *, discussion.created_date as dis_create from discussion 
//                           join users on users.user_id=discussion.user_id 
//                           join product on product.deal_id=discussion.deal_id where $contitions order by discussion_id DESC");
                return count($result);
        }
...
//                       join transaction_mapping as tm on tm.transaction_id = t.id 
//                       join city on city.city_id=s.city 
//                       join users as u on u.user_id=s.user_id where $contitions and shipping_type = 1 $condition group by shipping_id order by shipping_id DESC "); 
//		
...
//                            join city on city.city_id=s.city 
//                            join users as u on u.user_id=s.user_id 
//                            where shipping_type = 1 $condition group by shipping_id order by shipping_id DESC"); 
//		
...
//           
	...
	{
//		$result = $this->db->query("SELECT *,transaction.currency_code as curren_code FROM (`shipping_info`)  
//                    LEFT JOIN transaction ON transaction.id = shipping_info.transaction_id 
//                    WHERE `shipping_id` = ".
                          $result = $this->db->select("*,transaction.currency_code as curren_code")
                          ->from("shipping_info")
                          ->join("transaction","transaction.id","shipping_info.transaction_id","LEFT")
                         
                          
                         ->where("shipping_id= ".$id)->get();
...
/* COUNT OF STORE CREDIT TRANSACTION */
	...
	{
		$condition="";
//		$result = $this->db->query("select * from store_credit_save as s 
//                    join product on product.deal_id = s.productid 
//                    join users on users.user_id = s.userid where credit_status = $status $condition order by storecredit_id desc limit $offset,$record ");
                 ...
                 $result = $this->db->select("*")
                          ...
                          ->where("credit_status=".$status.$condition)
                         
                          ->orderby('storecredit_id','DESC')
                          ->limit($offset,$record)
                          ->get();
