	...
	public function get_all_deals_list($type = "",$offset = "", $record = "", $city = "", $name = "",$sort_type = "",$param = "",$limit = "",$today="", $startdate = "", $enddate = "")
	{
		...
		if($_GET){   
			...
			//$query = "select * , deals.created_date as createddate from deals join stores on stores.store_id=deals.shop_id join city on city.city_id=stores.city_id  
                        //    join country on country.country_id=stores.country_id join category on category.category_id=deals.category_id 
			...
			$result = $this->db->select("* , deals.created_date as createddate")
		      ...
		      //$query = "select * , deals.created_date as createddate from deals                       join stores on stores.store_id=deals.shop_id 
                          ...
                          //join users on users.user_id=deals.merchant_id 
                          //where $conditions order by deals.deal_id DESC $limit1";
                      $result = $this->db->select("* , deals.created_date as createddate")
                              ->from ("deals")
                              ->join ("stores","stores.store_id","deals.shop_id")
                              ->join ("city","city.city_id","stores.city_id")
                              ->join ("country","country.country_id","country_id")
                              ->join ("category","category.category_id","deals.category_id")
                              ->join ("users","users.user_id","deals.merchant_id")
                              ->where($conditions)
	...
	/** GET DEAL COMMENTS LIST  **/
        ...
        public function get_users_comments_list($offset = "", $record = "",  $firstname = "",$deal_type = "")
        {
                ...
                $contitions = "discussion.type = $deal_type ";
                      ...
                      // $result = $this->db->query("select *, discussion.created_date as dis_create from discussion join users on users.user_id=discussion.user_id 
                       //    join deals on deals.deal_id=discussion.deal_id where $contitions order by discussion_id DESC limit $offset, $record");
