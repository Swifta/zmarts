...
class Store_admin_Model extends Model
{
...
//               
	...
	public function merchant_login($email = "", $pswd = "")
	{
        ...
        if(count($result)>0){
			if($result->current()->user_status == 1){
				...
				if(count($res)>0){
					$this->merchant_id=$result->current()->referred_user_id;
					$this->merchant_id1=$result->current()->referred_user_id;
	...
	public function get_all_deals_list($type = "", $offset = "", $record = "", $city = "", $name = "",$sort_type = "",$param = "",$limit="",$today="", $startdate = "", $enddate = "")
	{
		...
		if($type == 1){
		 ...
		 $conditions = "deals.enddate $cont ".time()." AND deals.merchant_id = ".$this->user_id." and stores.store_id = ".$this->store_id." ";
                if($_GET){

                        if($city){
                                $conditions .= " and city.city_id = ".strip_tags(addslashes($city));
                        }
                        if($name){
...
//				$qry = "select * , deals.created_date as createddate 
//                                    from deals join stores on stores.store_id=deals.shop_id 
//                                    join city on city.city_id=stores.city_id 
//                                    join country on country.country_id=stores.country_id 
//                                    join category on category.category_id=deals.category_id 
...
//                                
		      ...
		      ->join("category","category.category_id","deals.category_id")
                       ->join("users","users.user_id","deals.merchant_id")
                      ->where($conditions. " order by deals.deal_id DESC")
                      ->limit($limit1)
                      ->get();
...
//                        $result = $this->db->query($qry);
			...
			$result = $this->db->select("deal_id")->from("deals")
						->join("stores","stores.store_id","deals.shop_id")
						->join("city","city.city_id","stores.city_id")
						->where(array("enddate $cont" => time(), "deals.merchant_id" => $this->user_id,"stores.store_id"=>$this->store_id))
						->get();
		}
		return count($result);
	}
	...
	public function get_deals_data($deal_key = "", $deal_id = "")
	{
				...
				->where(array("deal_id" => $deal_id, "deal_key" => $deal_key,"deals.merchant_id" => $this->user_id,"deals.shop_id"=>$this->store_id))
				->join("stores","stores.store_id","deals.shop_id")
				->join("city","city.city_id","stores.city_id")
				->join("country","country.country_id","stores.country_id")
				->join("users","users.user_id","deals.merchant_id")
...
//        
	...
	public function add_merchant_shop($post = "",$store_key = "",$pswd="")
	{
			...
			$res = $this->db->insert("users",array("firstname"=>$post->username,"email"=>$post->email,"password"=>md5($pswd),"user_type"=>9,"created_by"=>$this->user_id,"referred_user_id"=>$this->user_id,"user_status"=>1,"login_type"=>1,"approve_status"=>1,"address1"=>$post->address1,"address2"=>$post->address2,"city_id"=>$post->city,"country_id"=>$post->country, 'phone_number' => $post->mobile));
			
			$stores_result = $this->db->insert("stores", array("store_name" => $post->storename,"store_url_title" => url::title($post->storename),'store_key' =>$store_key,'address1' => $post->address1, 'address2' => $post->address2, 'city_id' => $post->city, 'country_id' => $post->country, 'phone_number' => $post->mobile, 'zipcode' => $post->zipcode, "meta_keywords" => $post->meta_keywords , "meta_description" =>  $post->meta_description,'website' => $website, 'latitude' => $post->latitude, 'longitude' => $post->longitude,'created_by'=>$this->user_id, 'store_type' => '2','merchant_id'=>$this->user_id,"created_date" => time(),"about_us"=>$post->about_us,"store_admin_id"=>$res->insert_id()));
...
//                        $result = $this->db->query("select * from stores join country on country.country_id = stores.country_id join city on city.city_id = stores.city_id 
...
//                             where $contitions ");
		...
		{
			$this->db->update("users",array("firstname"=>$post->username,"email"=>$post->email,"address1"=>$post->address1,"address2"=>$post->address2,"city_id"=>$post->city,"country_id"=>$post->country, 'phone_number' => $post->mobile),array("user_id" => $post->store_admin_id));
			$merchant_result = $this->db->update("stores", array("store_name" => $post->storename,"store_url_title" => url::title($post->storename),'address1' => $post->address1, 'address2' => $post->address2, 'city_id' => $post->city, 'country_id' => $post->country, 'phone_number' => $post->mobile, 'zipcode' => $post->zipcode, "meta_keywords" => $post->meta_keywords , "meta_description" =>  $post->meta_description,'website' => $post->website, 'latitude' => $post->latitude, 'longitude' => $post->longitude,"about_us"=>$post->about_us), array('store_id' => $id,"merchant_id" => $this->user_id));
	...
	public function get_all_products_count($type = "", $city = "", $name = "",$sort_type = "",$param = "",$today="", $startdate = "", $enddate = "",$product_duration="")
	{
		...
		if($_GET){
			...
			if($type != "1")
		        {
		                $conditions = "purchase_count < user_limit_quantity and product.merchant_id = ".$this->user_id." and stores.store_status = 1 and product.shop_id = ".$this->store_id." and deal_status!=2 ";
		        }else {
		                $conditions = "purchase_count = user_limit_quantity and product.merchant_id = ".$this->user_id." and stores.store_status = 1 and product.shop_id = ".$this->store_id." and deal_status!=2  ";
		        }
			if($city){
...
//                            join city on city.city_id=stores.city_id 
//                            join country on country.country_id=stores.country_id
//                            join category on category.category_id=product.category_id join users on users.user_id=product.merchant_id where $conditions  ";
//			$result = $this->db->query($qry);
...
//                            join stores on stores.store_id=product.shop_id 
//                            join city on city.city_id=stores.city_id  
//                            join country on country.country_id=stores.country_id join 
...
//                                            join users as u on u.user_id=s.user_id where $contitions and shipping_type = 1 AND d.merchant_id = $this->user_id and d.shop_id = ".$this->store_id." $condition group by shipping_id order by shipping_id DESC $limit1 ");
						...
						->where($contitions." and shipping_type = 1 AND d.merchant_id = $this->user_id and d.shop_id = ".$this->store_id." ".$condition)
                                                ->groupby("shipping_id")
...
//					$result = $this->db->query("select *,s.adderss1 as saddr1,s.address2 as saddr2,u.phone_number,t.id as trans_id,stores.address1 as addr1,stores.address2 as addr2,stores.phone_number as str_phone,t.shipping_amount as shipping,stores.city_id as str_city_id from 
//                                            shipping_info as s join transaction as t on t.id=s.transaction_id join product as d on d.deal_id=t.product_id join transaction_mapping as tm on tm.transaction_id = t.id 
//                                            join city on city.city_id=s.city join stores on stores.store_id = d.shop_id join users as u on u.user_id=s.user_id 
...
//			 $result = $this->db->query("select s.shipping_id  from shipping_info as s join transaction as t on t.id=s.transaction_id 
//                             join product as d on d.deal_id=t.product_id
//                             join transaction_mapping as tm on tm.transaction_id = t.id
//                             join city on city.city_id=s.city 
//                             join stores on stores.store_id = d.shop_id 
	...
	public function count_transaction_list($type = "", $search_key = "",$sort_type = "",$param = "",$trans_type="",$today="", $startdate = "", $enddate = "")
	{
		 ...
		 if($_GET){
			 $search_key = strip_tags(addslashes($search_key));
			  if(($type=="")||($type=="mail")) {
		                $conditions .= "transaction.id > 0 and auction.shop_id = ".$this->store_id." ";
		          }else {
		                $conditions .= "(payment_status='$type') and auction.shop_id = ".$this->store_id." ";
		          }
		           if($today == 1)
                        {
                                $from_date = date("Y-m-d 00:00:01"); 
...
//			$result = $this->db->query('select * from transaction join users on users.user_id=transaction.user_id 
//                            join auction on auction.deal_id=transaction.auction_id 
...
//                        
			...
			if(($type=="")||($type=="mail")) {
					...
					$conditions = "transaction.id >= 0 and auction.merchant_id = '$this->user_id' and auction.shop_id = ".$this->store_id." ";
				...
				if($trans_type){
					...
					$conditions = " payment_status = '$type' and auction.merchant_id = '$this->user_id' and auction.shop_id = ".$this->store_id." ";
	...
	public function count_transaction_deal_list($type = "", $search_key = "",$sort_type = "",$param = "",$trans_type = "",$today="", $startdate = "", $enddate = "")
	{
		 ...
		 if($_GET){
			 $search_key = strip_tags($search_key);
			  if(($type=="")||($type=="mail")) {
		                $conditions = "transaction.id > 0 and deals.shop_id = ".$this->store_id." ";
		          }else {
		                $conditions = "(payment_status='$type') and deals.shop_id = ".$this->store_id."";
		          }
		          if($trans_type){
				$conditions .= " AND transaction.type = 5 ";
			}
...
//			$result = $this->db->query('select *,users.firstname as firstname from transaction 
//                            join users on users.user_id=transaction.user_id 
//                            join deals on deals.deal_id=transaction.deal_id 
//                            where '.$conditions.' and (users.firstname like "%'.$search_key.'%" OR transaction.transaction_id like "%'.$search_key.'%" OR deals.deal_title like "%'.$search_key.'%")');
		 $result = $this->db->select("*,users.firstname as firstname")
			...
			$conditions = "transaction.id >= 0 and deals.merchant_id = '$this->user_id' and deals.shop_id = ".$this->store_id." ";
			if($trans_type){
				$conditions .= " AND transaction.type = 5 ";
			}
			else{
			...
			$conditions = " payment_status = '$type' and deals.merchant_id = '$this->user_id' and deals.shop_id = ".$this->store_id."";
...
//			 $result = $this->db->query("select *,users.firstname as firstname from transaction 
//                             join users on users.user_id=transaction.user_id join deals on deals.deal_id=transaction.deal_id 
//                             where $conditions and (users.firstname like '%".$search_key."%' OR transaction.transaction_id like '%".$search_key."%' OR deals.deal_title like '%".$search_key."%') order by transaction.id DESC  $limit1 "); 
//		
                 $result = $this->db->select("*,users.firstname as firstname")
                                                ->from("transaction")
                                                ->join("users","users.user_id","transaction.user_id")
...
//			$result = $this->db->query("select *,users.firstname as firstname from transaction 
//                            join users on users.user_id=transaction.user_id join deals on deals.deal_id=transaction.deal_id where $conditions $limit1 ");
	...
	public function count_transaction_product_list($type = "", $search_key = "",$sort_type = "",$param = "",$trans_type = "",$today="", $startdate = "", $enddate = "")
	{
		 ...
		 if($_GET){
			 $search_key = strip_tags(addslashes($search_key));
			  if(($type=="")||($type=="mail")) {
		                $conditions = "transaction.id > 0 and product.shop_id = ".$this->store_id." ";
		          }else {
		                $conditions = "(payment_status='$type') and product.shop_id = ".$this->store_id."";
		          }
		           if($today == 1)
                        {
                                $from_date = date("Y-m-d 00:00:01"); 
...
//			$result = $this->db->query("select transaction.id from transaction 
//                            join users on users.user_id=transaction.user_id 
//                            join product on product.deal_id=transaction.product_id where $conditions and product.merchant_id = $this->user_id  and ( users.firstname like '%".$search_key."%' OR transaction.transaction_id like '%".$search_key."%' OR product.deal_title like '%".$search_key."%' )");
//		
                        $result = $this->db->select("transaction.id")
                                                ->from("transaction")
                                                ->join("users","users.user_id","transaction.user_id")
...
//                            join users on users.user_id=transaction.user_id 
//                            join auction on auction.deal_id=transaction.auction_id 
//                            where $conditions and auction.merchant_id = $this->user_id and ( users.firstname like '%".$search_key."%' OR transaction.transaction_id like '%".$search_key."%' OR auction.deal_title like '%".$search_key."%' ) limit $offset,$record");
//		
                         $result = $this->db->select("*,users.firstname as firstname")
                                                ->from("transaction")
                                                ->join("users","users.user_id","transaction.user_id")
...
//			    $result = $this->db->query("select *,users.firstname as firstname, transaction.shipping_amount as shippingamount 
//                            from transaction join users on users.user_id=transaction.user_id 
//                            join product on product.deal_id=transaction.product_id 
...
//                        $qry = "select deals.*,transaction_mapping.coupon_code,transaction_mapping.coupon_code_status,transaction.type,transaction.id as trans_id,transaction.amount,transaction.referral_amount,transaction.quantity 
//                            from deals 
//                            join transaction on transaction.deal_id=deals.deal_id  
//                            join transaction_mapping on transaction_mapping.transaction_id=transaction.id and transaction_mapping.deal_id=transaction.deal_id 
//                            where $conditions and deals.expirydate > $time and merchant_id = '$this->user_id' and deals.shop_id=".$this->store_id." limit 1 ";
//                        $result = $this->db->query($qry);

                        
                         $result = $this->db->select("deals.*,transaction_mapping.coupon_code,transaction_mapping.coupon_code_status,transaction.type,transaction.id as trans_id,transaction.amount,transaction.referral_amount,transaction.quantity")
                                                ->from("deals")
                                                ->join("transaction","transaction.deal_id","deals.deal_id ")
						->join("transaction_mapping","transaction_mapping.transaction_id","transaction.id")
						->join("transaction_mapping","transaction_mapping.deal_id","transaction.deal_id")
						->where("$conditions and deals.expirydate > '.$time.' and merchant_id = '.$this->user_id.' and deals.shop_id=".$this->store_id."")
                                                ->limit(1)
						->get();  
			         return $result;
	...
	public function get_all_auction_list($type = "",$offset = "", $record = "", $city = "", $name = "",$sort_type = "",$param = "",$limit="",$today="", $startdate = "", $enddate = "")
	{
		...
		$conditions = "auction.enddate $cont ".time()." AND auction.enddate > 0 AND stores.store_status = 1 AND auction.merchant_id = $this->user_id AND city_status =1 AND country_status = 1 and auction.shop_id = ".$this->store_id." ";
...
//			$qry = "select * , auction.created_date as createddate from auction join stores on stores.store_id=auction.shop_id 
//                            join city on city.city_id=stores.city_id  join country on country.country_id=stores.country_id 
//                            join category on category.category_id=auction.category_id 
//                            join users on users.user_id=auction.merchant_id where $conditions $limit1 ";
...
//                            join country on country.country_id=stores.country_id 
//                            join category on category.category_id=auction.category_id 
//                            join users on users.user_id=auction.merchant_id where $conditions order by auction.deal_id DESC $limit1 ";
//		
                        $result = $this->db->select("*, auction.created_date as createddate")
                                                ->from("auction")
                                                ->join("stores","stores.store_id","auction.shop_id")
	...
	public function get_all_auction_count($type = "", $city = "", $name = "",$sort_type = "",$param = "",$today="", $startdate = "", $enddate = "")
	{
			...
			if($sort_type == "DESC" ){
                ...
                if($_GET){
                        ...
                        $conditions = "auction.enddate $cont ".time()."  AND auction.enddate > 0 AND stores.store_status = 1 AND auction.merchant_id = $this->user_id and auction.shop_id = ".$this->store_id." ";
                        if($city){
                                $conditions .= " and city.city_id = ".strip_tags(addslashes($city));
                        }
...
//                                   $qry = "select * from auction join stores on stores.store_id=auction.shop_id 
//                                    join city on city.city_id=stores.city_id 
//                                    join category on category.category_id=auction.category_id 
//                                    join users on users.user_id=auction.merchant_id   where $conditions";
                                           // $result = $this->db->query($qry);
                        ...
                        $result = $this->db->select("deal_id")->from("auction")
                                        ->where(array("enddate $cont" => time(),"stores.store_status" => "1","auction.merchant_id" =>$this->user_id,"auction.shop_id"=>$this->store_id))
			                            ->join("stores","stores.store_id","auction.shop_id")
			                            ->join("city","city.city_id","stores.city_id")
			                            ->join("country","country.country_id","stores.country_id")
						    			->join("category","category.category_id","auction.category_id")
...
//                       join stores on stores.store_id = d.shop_id 
//                       join users as u on u.user_id=s.user_id 
//                       where $contitions and shipping_type = 2 and d.shop_id = ".$this->store_id." $condition group by shipping_id order by shipping_id DESC  $limit1 ");
//				
						 ...
						 ->join("auction as d","d.deal_id","t.auction_id")
                                                 ->join("transaction_mapping as tm","tm.transaction_id","t.id")
                                                 ->join("city","city.city_id","s.city join")
                                                 ->join("stores","stores.store_id","d.shop_id")
                                                 ->join("users","u.user_id","s.user_id")
                                                 ->where("$contitions and shipping_type = 2 and d.shop_id = ".$this->store_id." $condition group by shipping_id order by shipping_id DESC")
                                                 ->limit($limit1)
...
//                                            join stores on stores.store_id = d.shop_id 
//                                            join users as u on u.user_id=s.user_id  
//                                            where shipping_type = 2 and d.shop_id = ".$this->store_id."  $condition group by shipping_id order by shipping_id DESC $limit1 ");
						 ...
						 ->join("auction as d","d.deal_id","t.auction_id")
                                                 ...
                                                 ->join("stores","stores.store_id","d.shop_id")
                                                 ->join("users","u.user_id","s.user_id")
                                                 ->where("shipping_type = 2 and d.shop_id = ".$this->store_id."'.$condition.' ")
                                                 ->groupby("shipping_id")
                                                 ->orderby("shipping_id","DESC")
                                                 ->limit($limit1)
...
//                            join city on city.city_id=s.city 
//                            join users as u on u.user_id=s.user_id 
//                            where shipping_type = 2 and d.shop_id = ".$this->store_id." $condition group by shipping_id order by shipping_id DESC");
//		
//                        
                         $result = $this->db->select(" s.shipping_id")
                                                  ->from("shipping_info as s")
...
//                    join users on users.user_id=auction.winner j
//                    oin city on city.city_id=users.city_id 
...
//		       $contitions .= ' and (users.firstname like "%'.strip_tags(addslashes($name)).'%"';
//                       $contitions .= ' OR auction.deal_title like "%'.strip_tags(addslashes($name)).'%")';
//                       $result = $this->db->query("SELECT count(bidding.bid_id) as count FROM auction 
//                           join users on users.user_id=auction.winner 
//                           join city on city.city_id=users.city_id 
//                           join country on country.country_id=users.country_id join bidding on bidding.auction_id = auction.deal_id where $contitions ");
//			
                                 $result = $this->db->select("count(bidding.bid_id) as count")
                                                        ->from("auction")
                                                        ->join("users"," users.user_id","auction.winner")
                                                        ->join("city","city.city_id","users.city_id")
                                                        ->join("country","country.country_id","users.country_id")
...
//                    join city on city.city_id=users.city_id 
//                    join country on country.country_id=users.country_id 
//                    join bidding on bidding.auction_id = auction.deal_id where $contitions " );
//		
                  $result = $this->db->select("count(bidding.bid_id) as count")
                                                        ->from("auction")
                                                        ->join("users"," users.user_id","auction.winner")
	...
	public function forgot_password($email = "", $pswd = "")
	{
		...
		if(count($result) > 0){
			...
			$this->db->update("users",array("password" => md5($pswd) ), array("user_id" => $userid));
			return 1;
		}
		else{
			return 0;
...
//		$qry = "SELECT * FROM size ORDER BY CAST(size_name as SIGNED INTEGER) ASC";
                $result = $this->db->select("")
                                                        ->from("size")
                                                        ->orderby("CAST(size_name as SIGNED INTEGER) ASC")->get();  
...
//                            join city on city.city_id=s.city 
//                            join stores on stores.store_id = d.shop_id 
//                            join users as u on u.user_id=s.user_id  where shipping_type = 1  group by shipping_id order by shipping_id DESC");
//            return $result;
			...
			if(count($get_detail)){
				...
				$this->db->update('transaction_mapping',array('coupon_code_status' => 0),array("transaction_id" => $trans_id));
					...
					//$this->db->query("update product_size set quantity = quantity + $quantity where deal_id = '$product_id' and size_id = '$size_id' ");
                        ...
                        $this->db->update("product_size", array("quantity"=>new Database_Expression('quantity + '.$quantity)),array("deal_id"=>$product_id,"size_id" => $size_id));
							
 
 
 //$this->db->query("update product set user_limit_quantity = user_limit_quantity + $quantity where deal_id = '$product_id'");
 $this->db->update("product", array("user_limit_quantity"=>new Database_Expression('user_limit_quantity + '.$quantity)),array("deal_id"=>$product_id));
...
$this->db->update("users", array("merchant_account_balance"=>new Database_Expression('merchant_account_balance + '.$merchantcommission)),array("user_type"=> 3,"user_id"=> $merchant_id));
				...
				$this->db->update('transaction_mapping',array('coupon_code_status' => 0),array("transaction_id" => $trans_id));
						...
						//$result = $this->db->query("UPDATE auction SET enddate = $time,winner = 0,auction_status = 0 WHERE deal_id = $auction_id");
                                                $this->db->update("auction", array("enddate"=> $time,"winner" =>0,"auction_status"=> 0),array("deal_id"=> $auction_id));
	...
	public function get_merchant_details($shop_name="")
        {
//                $qry = "select * from users left join stores ON users.user_id=stores.merchant_id where stores.store_name='$shop_name' AND users.user_id=$this->user_id";
//                $result = $this->db->query($qry);                     
//                return $result;
                
                       $result =  $this->db->select()->from("users")
                       ->join("stores","users.user_id","stores.merchant_id","LEFT")
                       ->where(array("stores.store_name" => $shop_name, "users.user_id" => $this->user_id));
        ...
        public function get_merchant_and_shop_status($shop_name="")
	{
               ...
               // $qry = "select * from stores where store_name='$shop_name' AND merchant_id='$merchant_id'";
                 $qry =  $this->db->select()->from("stores")
	...
	{ 
	   ...
	   public function get_sub_group($subgroupname="",$maingroupid="")
	   {
		...
		{
			$result = $this->db->select("store_url_title")->from("stores")->where(array("store_id"=>$this->store_id,"merchant_id"=>$this->user_id))->get();
			return $result->current()->store_url_title;
		}
	...
	public function update_cms($post = "")
	{
		...
		$result = $this->db->update("stores",array("terms_conditions" => $post->terms_conditions, "terms_conditions_status" => $post->terms_conditions_status), array("merchant_id" => $this->user_id,"stores.store_id"=>$this->store_id));
		}
		...
		$result = $this->db->update("stores",array("return_policy" => $post->return_policy, "return_policy_status" => $post->return_policy_status), array("merchant_id" => $this->user_id,"stores.store_id"=>$this->store_id));
		}
		...
		$result = $this->db->update("stores",array("warranty" => $post->about_us, "warranty_status" => $post->about_us_status), array("merchant_id" => $this->user_id,"stores.store_id"=>$this->store_id));
		}
	...
	public function update_merchant_attribute($post = "")
	{
		$query_result=$this->db->select("store_id")->from("stores")->where(array("store_admin_id" => $this->store_admin_id))->get();
		$store_id=$query_result->current()->store_id;
		$result = $this->db->update("merchant_attribute",array("bg_color" => $post->bg_color, "font_color" => $post->font_color, "font_size" => $post->font_size, "banner_1_link" => $post->banner_1_link, "banner_2_link" =>$post->banner_2_link, "banner_3_link" => $post->banner_3_link, "ads_1_link" => $post->ads_1_link, "ads_2_link" =>$post->ads_2_link, "ads_3_link" => $post->ads_3_link), array("merchant_id" => $this->user_id,"storeid" =>$store_id));
                ...
                $user_result = $this->db->update("stores", array("store_sector_id" =>$post->sector,"store_subsector_id" =>$post->subsector), array("store_admin_id" => $this->store_admin_id));
		return $store_id;
	}
	...
	/** GET MODERATOR COUNT DATA  **/
        ...
        { 
			...
			if($sort_type == "DESC" ){
                ...
                if($_GET){
                        if($city){
                            //$contitions .= ' and login_type = '.$city;
...
//		return $result;
	...
	public function get_user_list1($all_users="",$city="",$gender="",$age_range="")
	{
		...
		{
		$news=$this->db->select("email","firstname")->from("users")->where(array("user_status"=>1,"user_type"=>4))->get();
		return $news;
		}
...
//			return $news;
                       ...
                       ->where(array("city_status" => 1, "user_type" => 4));
...
/** EXIST SECTOR DATA **/
	...
	{
		$result=$this->db->select("gift_name","gift_id")->from("free_gift")->where(array("merchant_id"=>$this->user_id1))->get();
		return $result;
	}
	...
	public function get_merchant_store_id()
	{	
		$result=$this->db->select("store_id")->from("stores")->where(array("store_admin_id"=>$this->store_admin_id))->get();
...
//                
	...
	public function get_merchant_store_name()
	{
		$result=$this->db->select("store_name")->from("stores")->where(array("store_id" =>$this->store_id))->get();
		return $result->current()->store_name;
		
	}
	
	public function get_store_name1() 
	{
		$result = $this->db->select("store_url_title","store_subsector_id")->from("stores")->where(array("store_id"=>$this->store_id,"store_admin_id"=>$this->store_admin_id))->get();
		return $result;
	}
	...
	{
		$result = $this->db->select("storecredit_id")->from("store_credit_save")->join("product","product.deal_id","store_credit_save.productid")->where(array("credit_status"=>$status,"merchantid"=>$this->user_id,"product.shop_id"=>$this->store_id))->get();
		return count($result);
	}
	...
	{
		$store_id = $this->store_id;
//		$result = $this->db->query("select *,s.shipping_amount from product left join store_credit_save as s on productid=deal_id 
//                    where s.storecredit_id = $storecreditid and deal_id = $productid and product.shop_id= $store_id ");
//		return $result;
                
                 $result =  $this->db->select("*,s.shipping_amount")->from("product")
                         ->join("store_credit_save","productid","deal_id","LEFT")
                       ->where(array("s.storecredit_id" => $storecreditid, "deal_id" => $productid,"product.shop_id"=>$store_id));
