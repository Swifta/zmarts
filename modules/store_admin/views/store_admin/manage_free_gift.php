        ...
        <?php $this->session->set("lasturl",substr($_SERVER['REQUEST_URI'], REQUEST_URL_COUNT));  ?>
            ...
            <tr>
                ...
                <td><?php if($c->gift_type==0){ echo $this->Lang["FREE_GIFT"]; } else { echo $this->Lang["AMOUNT_BASED"]." ( ". CURRENCY_SYMBOL.$c->gift_Amount ." ) "; } ;?></td>
                    <td>		
                    	<a href="<?php echo PATH.'store-admin/edit-gift/'.$c->gift_id.'/'.$c->merchant_id;?>.html" class="editicon" title="<?php echo $this->Lang['EDIT_GIFT']; ?>"></a>
                    </td>
                    <td>		
                    	<?php if($c->gift_status == 1){?>
                    	<a onclick="return blockunblockGift_storeadmin('<?php echo $c->gift_id; ?>','<?php echo $c->merchant_id; ?>','block');" class="blockicon" title="<?php echo $this->Lang['BLO_GIFT']; ?>"></a>
                        <?php } else{  ?>
                        <a onclick="return blockunblockGift_storeadmin('<?php echo $c->gift_id; ?>','<?php echo $c->merchant_id; ?>','unblock');" class="unblockicon" title="<?php echo $this->Lang['UNBLO_GIFT']; ?>"></a>
                        <?php } ?>
