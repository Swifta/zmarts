    ...
    <div class="content_middle">
        ...
        <?php $this->session->set("lasturl",substr($_SERVER['REQUEST_URI'], REQUEST_URL_COUNT));  ?>
            ...
            <tr>
				...
				<td>
					<a href="<?php echo PATH.'merchant/edit-template/'.base64_encode($news->newsletter_id).'.html';?>" class="editicon" title="<?php echo $this->Lang['EDIT_TEMPLATE']; ?>"></a>
				</td>
				<td>
					<?php if($news->newsletter_status==1){?>
					<a onclick="return blockunblockMerchantNewsletter('<?php echo base64_encode($news->newsletter_id); ?>','block');" class="blockicon" title="<?php echo $this->Lang['BLOCK']; ?>"></a>
					<?php } else{  ?>
					<a onclick="return blockunblockMerchantNewsletter('<?php echo base64_encode($news->newsletter_id); ?>','unblock');" class="unblockicon" title="<?php echo $this->Lang['UNBLOCK']; ?>"></a>
					<?php } ?>
				</td>
				<td>
					<a onclick="return deleteMerchantNewsletter('<?php echo base64_encode($news->newsletter_id); ?>');" class="deleteicon" title="<?php echo $this->Lang['DELETE']; ?>" ></a>
				</td>
            </tr>
            <?php $i++; }}else{?>
            <tr>
