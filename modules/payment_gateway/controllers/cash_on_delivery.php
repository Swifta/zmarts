...
class Cash_on_delivery_Controller extends Layout_Controller
{
	...
	{
		parent::__construct();
		$this->cod = new Cash_on_delivery_Model;
	}
	...
	public function product_payment()
	{
        ...
        if(!is_array($value)){            
            if(($key=='product_cart_id'.$value)){
                ...
                $this->product_size_details = $this->cod->product_size_details($deal_id, $product_size);
		...
		if($_POST){
			...
			foreach($_SESSION as $key=>$value)
			{
				...
				if(($key=='product_cart_id'.$value)){
					...
					$this->gift_type=$this->cod->get_product_gift_type($merchant_id,$deal_id,$gift_offer);
					$gift_type=isset($this->gift_type[0]->gift_type)?$this->gift_type[0]->gift_type:0;
					 ...
					 if($shipping_methods  == 1){
							...
							$merchant_flat_amount = $this->cod->get_userflat_amount($merchant_id);
					...
					if($this->session->get('prime_customer')==1){
								//if(array_key_exists($merchant_id,$cart_merchant)){
									...
									{
													$this->free_gift=$this->cod->get_free_gift($cart_product_amount,$merchant_id,$deal_id);
												}elseif($product_offer==2 && $gift_type==0 &&  $end_date > time() && $start_date < time()){
												
													$this->free_gift=$this->cod->get_free_gft_not_per_amount($merchant_id,$deal_id,$product_offer);
												}
			...
			url::redirect(PATH."payment_product/cod_transaction.html");
		}
	}
	...
	public function do_captured_transaction($captured = "", $deal_id = "", $qty = "",$transaction = "")
	{
		...
		foreach($user_details as $U){
			...
			$deals_details = $this->cod->get_deals_details($deal_id);
	...
	public function do_captured_transaction1($captured = "", $deal_id = "", $qty = "",$transaction = "",$transid ="")
	{
		...
		foreach($user_details as $U){
            ...
            if($friend_email != "xxxyyy@zzz.com"){
                ...
                $this->admin_list = $this->cod->get_admin_list();
	...
	public function deal_cash_delivery()
	{
		if($_POST){
			...
			$this->deals_payment_deatils = $this->cod->get_deals_payment_details($deal_id, $deal_key);

			if(count($this->deals_payment_deatils) == 0){
				common::message(-1, $this->Lang["PAGE_NOT"]);
				url::redirect(PATH);
			}
			$this->referral_balance_deatils = $this->cod->get_user_referral_balance_details();
			$this->get_user_limit_details = $this->cod->get_user_limit_details($deal_id);
			$this->get_user_limit_count = $this->cod->get_user_limit_details($deal_id);
			$this->user_referral_balance = $this->cod->get_user_referral_balance_details();
	...
	private function do_captured_transaction_deal($captured = "", $deal_id = "", $qty = "")
	{
		if($captured == 0){
			$this->set_pay_later = 1;
			$captured_list = $this->cod->payment_authorization_list($deal_id);
			foreach($captured_list as $C){
				if(($C->type==1)||($C->type==2)){  // for COD transaction
					...
					if($result->ACK = "Success" ){
							$status = $this->cod->update_captured_transaction($result, $C->id);
					}
				} else {
						$status = $this->cod->update_captured_transaction_success($C->id);
				}
			}
			$captured_mail_list = $this->cod->payment_authorization_mail_list($deal_id);
			foreach($captured_mail_list as $mail){
				$friend_details = $this->cod->get_friend_transaction_details_deal($deal_id, $mail->id);
				$friend_email = $friend_details->current()->friend_email;
				$friend_name = $friend_details->current()->friend_name;
				...
				$transaction_coupon_details = $this->cod->get_all_deal_captured_coupon($deal_id, $mail->user_id, $mail->order_date);
				$coupon_array = array();
				$coupon_code = "";
				...
				if($friend_email != "xxxyyy@zzz.com"){
					$store_detail = $this->cod->get_payment_store_detail($deal_id);
				...
				$merchant_email = ""; 
				$this->get_merchant_details = $this->cod->get_merchant_details($mail->merchant_id);
				...
				$transaction_coupon_update = $this->cod->update_transaction_coupon_status1($deal_id,$mail->user_id,$mail->order_date);
				for($i=0; $i < count($coupon_array); $i++){
	...
	private function payment_mail_function_deal( $captured = "", $deal_id = "", $transaction_id = "")
	{
	    if($captured == 1){
			$this->set_pay_later = 1;
			$from = CONTACT_EMAIL;
			$transaction_details = $this->cod->get_all_deal_captured_transaction($deal_id, $transaction_id, $captured);
			foreach($transaction_details as $TD){
				$friend_details = $this->cod->get_friend_transaction_details_deal($deal_id, $TD->id);
				$friend_email = $friend_details->current()->friend_email;
				$friend_name = $friend_details->current()->friend_name;
				...
				$merchant_email = ""; 
				$this->get_merchant_details = $this->cod->get_merchant_details($TD->merchant_id);
				$merchant_email = $this->get_merchant_details->current()->email;
				$this->merchant_firstname = $this->get_merchant_details->current()->firstname;
				$this->merchant_lastname = $this->get_merchant_details->current()->lastname;
				$message_merchant = new View("themes/".THEME_NAME."/payment_mail_merchant");
	...
	{ 
		if($_POST){  
			...
			$transaction = $this->cod->insert_auction_pay_transaction_details($deal_id,$referral_amount, $item_qty, 6, $captured, $item_qty, $post,$merchant_id,$tax_amount,$shipping_amount,$amount,$bid_id);	
	...
	{
		$this->auction_details = $this->cod->get_auction_mail_data($deal_id,$transaction_id);
		$this->set_pay_later = 1;
