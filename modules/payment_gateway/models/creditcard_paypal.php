...
class Creditcard_paypal_Model extends Model
{
	...
	{	
		...
		(strcmp($_SESSION['Club'], '1') == 0)?$this->deal_value_condition_field = 'product.deal_prime_value':$this->deal_value_condition_field = 'product.deal_value';
			...
			(strcmp($_SESSION['Club'], '1') == 0)?$this->deal_value_condition_p = 'product.deal_prime_value as deal_value':$this->deal_value_condition = 'product.deal_value';
	...
	public function insert_transaction_details($R = "", $deal_id = "", $country_code = "", $firstName = "", $lastName = "", $ref_amount = 0, $qty = 1, $type = 1, $captured = 0, $purchase_qty = "" ,$post = "",$merchant_id = "",$friend_gift = "",$friendName = "",$friendEmail = "",$product_size = "",$product_color = "",$tax_amount = "" ,$shipping_amount = "", $deal_amount = "", $shipping_methods = "",$bulk_discount="",$free_gift="",$prime_customer=0,$bulk_discount1,$total_bulk_discount="",$product_offer="",$gift_type="")
	{
...
//	    return $result;
	...
	public function get_products_merchant_list($trans_id="",$merchant_id = "", $type="") 
	{
		...
		/*
			$result = $this->db->select("*",$this->deal_value_condition,'s.adderss1 as saddr1','s.address2 as saddr2','u.phone_number','t.id as trans_id','stores.address1 as addr1','stores.address2 as addr2','stores.phone_number as str_phone', 't.shipping_amount as shipping','stores.city_id as str_city_id')->from("product")->join("transaction as t","product.deal_id", "t.product_id")->join("shipping_info as s","t.id","s.transaction_id")
		//->join("product","product.deal_id","t.product_id")
		->join("city","city.city_id","s.city")
		->join("stores","stores.store_id","product.shop_id")
		->join("users as u", "u.user_id","s.user_id")->where(array("shipping_type"=>1, "t.transaction_id"=>"$trans_id", "t.type"=>"5", "product.merchant_id"=>"$merchant_id"))->get();*/
		
		
		/*$result = $this->db->query("select *,$this->deal_value_condition,s.adderss1 as saddr1,s.address2 as saddr2,u.phone_number,t.id as trans_id,stores.address1 as addr1,stores.address2 as addr2,stores.phone_number as str_phone,t.shipping_amount as shipping,stores.city_id as str_city_id,t.bulk_discount,t.store_credit_period  from shipping_info as s join transaction as t on t.id=s.transaction_id join product as d on product.deal_id=t.product_id join city on city.city_id=s.city join stores on stores.store_id = product.shop_id join users as u on u.user_id=s.user_id  where shipping_type = 1 and t.transaction_id ='$trans_id' and product.merchant_id ='$merchant_id' $condition order by shipping_id DESC ");*/
		...
		$result = $this->db->select("*,$this->deal_value_condition,s.adderss1 as saddr1,s.address2 as saddr2,u.phone_number,t.id as trans_id,stores.address1 as addr1,stores.address2 as addr2,stores.phone_number as str_phone,t.shipping_amount as shipping,stores.city_id as str_city_id,t.bulk_discount,t.store_credit_period")
                            ...
                            ->join("stores","stores.store_id","product.shop_id")
                            ->join("users as u","u.user_id","s.user_id")
                            ->where("shipping_type = 1 and t.transaction_id ='". strip_tags(addslashes($trans_id))."' and product.merchant_id ='". strip_tags(addslashes($merchant_id))."' ".$condition)
                            ->orderby("shipping_id", "DESC")
                            ->get();
		return $result;
	}
	...
	{
		$resulkt=$this->db->select("gift_name,gift_id,gift_description")->from("free_gift")->where(array("gift_status" => 1,"gift_id" =>$gift_id))->get();
		return $resulkt;
	}
