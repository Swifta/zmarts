...
global $API_Endpoint,$version,$API_UserName,$API_Pswd,$API_Signature,$nvp_Header, $subject, $AUTH_token,$AUTH_signature,$AUTH_timestamp;
...
if(defined('AUTH_MODE')) {
	...
	if((!empty($API_UserName)) && (!empty($API_Pswd)) && (!empty($API_Signature)) && (!empty($subject))) {
		$AuthMode = "THIRDPARTY";
	}
	
	else if((!empty($API_UserName)) && (!empty($API_Pswd)) && (!empty($API_Signature))) {
		$AuthMode = "3TOKEN";
	}
...
switch($AuthMode) {
	...
	case "3TOKEN" : 
			$nvpHeaderStr = "&PWD=".urlencode($API_Pswd)."&USER=".urlencode($API_UserName)."&SIGNATURE=".urlencode($API_Signature);
			break;
	case "FIRSTPARTY" :
			$nvpHeaderStr = "&SUBJECT=".urlencode($subject);
			break;
	case "THIRDPARTY" :
			$nvpHeaderStr = "&PWD=".urlencode($API_Pswd)."&USER=".urlencode($API_UserName)."&SIGNATURE=".urlencode($API_Signature)."&SUBJECT=".urlencode($subject);
			break;		
	case "PERMISSION" :
